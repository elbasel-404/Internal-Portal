...
import { EmployeeMembersRequest } from '@types';

interface EmployeeMembersProps {
  data: EmployeeMembersRequest[];
}
...
const tableHeaders = [
  ...
  { label: 'نوع الطلب' },
  { label: 'صلة القرابة' },
  { label: 'الاسم الكامل للفرد' },
  { label: 'اسم الفرد بالإنجليزية' },
  { label: 'رقم الهوية' },
  { label: 'تاريخ الميلاد' },
  { label: 'الحالة' },
];

export const EmployeeMembersTable = ({ data }: EmployeeMembersProps) => {
      ...
      <div className='bg-white rounded-lg'>
        <div className='flex flex-col md:flex-row gap-4 items-start md:items-center justify-between border-b border-border-[#ECF0F480] p-4'>
          <h2 className='text-foreground font-bold text-2xl'>
            قائمة طلبات تحديث أفراد الأسرة
          </h2>
          <div className='flex gap-4 items-center'>
            ...
            <Link
              href={paths.employeeMembersNew.href}
        ...
        <Table
          ...
          link={paths.employeeMembersDetails.href}
        />
      </div>
    </>
  );
